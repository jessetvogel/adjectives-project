    ...
    try {
        ...
        for (const type in book.theorems)
            for (const id in book.theorems[type])
                update_json_file(`${PATH_JSON}/theorems/${type}/${id}.json`, book.serialize_adjective(book.theorems[type][id], true));
        for (const type in book.examples)
            for (const id in book.examples[type])
                update_json_file(`${PATH_JSON}/examples/${type}/${id}.json`, book.serialize_adjective(book.examples[type][id], true));
    }
    catch (err) {
        console.log(err);
    }
...
main();
//# sourceMappingURL=script-dump-json.js.map